...
contract('ZapIn', accounts => {
  ...
  describe('ZapIn Contract Testing', async () => {
    beforeEach('basic setup', async () => {
      ...
      let factory = await SLSFactory.new(accounts[0]);
    ...
    it('#zapIn', async () => {
      ...
      let swapAmounts = await zapIn.calculateSwapAmounts(token.address, weth.address, pair.address, userIn);
      let result = await zapIn.zapIn(token.address, weth.address, userIn, pair.address, accounts[1], 1, MaxUint256, {
        from: accounts[1]
      });
    ...
    it('#zapInEth', async () => {
      ...
      await zapIn.zapInEth(token.address, pair.address, accounts[1], 1, MaxUint256, {
        from: accounts[1],
        value: userIn
      });
    ...
    it('#zapOut', async () => {
      ...
      let zapOutAmount = await zapIn.calculateZapOutAmount(token.address, weth.address, pair.address, liquidity);

      let beforeBalance = await Helper.getBalancePromise(accounts[1]);
      await zapIn.zapOutEth(token.address, liquidity, pair.address, accounts[1], 1, MaxUint256, {
        from: accounts[1],
        gasPrice: new BN(0)
      });

...
module.exports.getAmountOut = async (amountIn, tokenIn, pair) => {
  ...
  let reserveIn =
    token0Addr == tokenIn.address ? tradeInfo._reserve0 : tradeInfo._reserve1;
  let reserveOut =
    token0Addr == tokenIn.address ? tradeInfo._reserve1 : tradeInfo._reserve0;
  ...
  return reserveOut.sub(
    numerator.add(denominator.sub(new BN(1))).div(denominator)
  );
};
...
module.exports.getPriceRange = (tradeInfo) => {
  ...
  if (tradeInfo._reserve0.eq(tradeInfo._reserve0)) {
    ...
    let limVReserve1 = tradeInfo._reserve1
      .mul(tradeInfo._reserve0)
      .div(limVReserve0);
  ...
  if (tradeInfo._reserve1.eq(tradeInfo._reserve1)) {
    ...
    let limVReserve0 = tradeInfo._reserve1
      .mul(tradeInfo._reserve0)
      .div(limVReserve1);

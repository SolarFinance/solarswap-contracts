...
contract("SolarswapFactory", function (accounts) {
  before("init", async () => {
    ...
    feeTo = accounts[2];
    admin = accounts[3];
    factory = await SolarswapFactory.new(feeToSetter, admin);

    tokenA = await TestToken.new(
      "test token A",
      "A",
      Helper.expandTo18Decimals(10000)
    );
    tokenB = await TestToken.new(
      "test token B",
      "B",
      Helper.expandTo18Decimals(10000)
    );
  ...
  it("create pair", async () => {
    await expectRevert(
      factory.createPair(tokenA.address, constants.ZERO_ADDRESS),
      "Solarswap: ZERO_ADDRESS"
    );

    await expectRevert(
      factory.createPair(tokenA.address, tokenA.address),
      "Solarswap: IDENTICAL_ADDRESSES"
    );
  ...
  it("set feeToSetter", async () => {
    ...
    await factory.setFeeToSetter(newFeeToSetter, { from: admin });

    assert(
      (await factory.feeToSetter()) == newFeeToSetter,
      "unexpected feeToSetter"
    );
  });
});

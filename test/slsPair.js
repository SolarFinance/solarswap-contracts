...
contract('SolarswapPair', function (accounts) {
  ...
  //   assert(await pair.symbol(), `Solarswap-LP ${await token0.symbol()} ${await token1.symbol()}`, 'unexpected symbol');
  //   assert(await pair.name(), `Solarswap LP ${await token0.symbol()} ${await token1.symbol()}`, 'unexpected name');
  // });

  it('can not initialize not by factory', async () => {
    [factory, pair] = await setupPair(admin, token0, token1);
    await expectRevert(pair.initialize(token0.address, token1.address), 'Solarswap: FORBIDDEN');
  });
  ...
  describe('mint', async () => {
    it('unamplified pair', async () => {
      ...
      await expectRevert(pair.mint(trader, {from: app}), 'Solarswap: INSUFFICIENT_LIQUIDITY_MINTED');
  ...
  describe('swap', async () => {
    ...
    swapTestCases.forEach((testCase, i) => {
      ...
      it(`getInputPrice:${i} unamplified pair`, async () => {
        ...
        let expectedOutputAmount = await slsHelper.getAmountOut(expandTo18Decimals(swapAmount), token0, pair);
        // console.log('expectedOutputAmount :>> ', expectedOutputAmount.toString());
        await expectRevert(pair.swap(0, expectedOutputAmount.add(new BN(1)), trader, '0x'), 'Solarswap: K');
    ...
    it('swap:token0 unamplified pair', async () => {
      ...
      let amountOut = await slsHelper.getAmountOut(swapAmount, token0.address, pair);
      // when amountIn = 0 -> revert
      await expectRevert(pair.swap(new BN(0), amountOut, trader, '0x', {from: app}), 'Solarswap: INSUFFICIENT_INPUT_AMOUNT');
      ...
      await expectRevert(pair.swap(new BN(0), amountOut, token0.address, '0x', {from: app}), 'Solarswap: INVALID_TO');
      ...
      console.log(`unamplified pair swap gasUsed = ${txResult.receipt.gasUsed}`);
    });
    ...
    optimisticTestCases.forEach((testCase, i) => {
      it(`optimistic:${i}`, async () => {
        ...
        let outputAmount = inputAmount.mul(Helper.precisionUnits.mul(new BN(998))).div(Helper.precisionUnits.mul(new BN(1000)));
        await expectRevert(pair.swap(outputAmount.add(new BN(1)), 0, trader, '0x'), 'Solarswap: K');
  ...
  describe('burn', async () => {
    it('burn unamplified pair', async () => {
      ...
      await expectRevert(pair.burn(liquidityProvider, {from: app}), 'Solarswap: INSUFFICIENT_LIQUIDITY_BURNED');
...
async function setupFactory(admin) {
  return await SLSFactory.new(admin);
}
